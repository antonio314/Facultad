...
int main(void) {
	 ...
	 double tiempo;
         int indice;
/*
inicializar mpi (mpi_init)
num procesadores (comm_size)
id proc (comm_rank)
*/
	 fi=fopen("julia.raw","wb");
	 if (!fi)
	    {
         ...
         cr=-0.8;   
         ci=0.156;     
	 t0=clock();  //para coger el tiempo tiempo=mpi_wtime();
	 for (j=0;j<Fil;j++)
	     for (i=0;i<Col;i++)
	         {
		  zi=(float)Venty+(float)j*Incy; // z0=coordenadas del punto (parte imaginaria).
	          ...
	          color=0; // Cada punto se colorea según el número de iteraciones necesarias para escapar.
                  indice=0; //Se contabiliza el número de iteraciones utilizando un valor entero y no char.
...
/*
comprobar si es proceso 0 o no y hacer los send y los recieve
